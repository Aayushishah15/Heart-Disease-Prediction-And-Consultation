...
def prdicts_heart_disease(list_data):
    csv_file = Admin_Helath_CSV.objects.get(id=1)
    df = pd.read_csv(csv_file.csv_file)
    ...
    y = df['target']
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.25, random_state=0)
    nn_model = LogisticRegression(random_state=0)  
    nn_model.fit(X_train, y_train)
    preds = nn_model.predict([list_data])
    print("Neural Network Accuracy: {:.2f}%".format(nn_model.score(X_test, y_test) * 100))
    print("Prdicted Value is : ", format(preds))
    dataframe = str(df.head())
    return (nn_model.score(X_test, y_test) * 100),(preds)

def prdictss_heart_disease(list_data):
    csv_file = Admin_Helath_CSV.objects.get(id=1)
    df = pd.read_csv(csv_file.csv_file)
    ...
    y = df['target']
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.25, random_state=0)
    nn_model =RandomForestClassifier(n_estimators= 10, criterion="entropy")  
    nn_model.fit(X_train, y_train)
    predss = nn_model.predict([list_data])
    print("Neural Network Accuracy: {:.2f}%".format(nn_model.score(X_test, y_test) * 100))
    print("Prdicted Value is : ", format(predss))
    dataframe = str(df.head())
    return (nn_model.score(X_test, y_test) * 100),(predss)
...
def add_heartdetail(request):
    if request.method == "POST":
        ...
        # list_data = [57, 0, 1, 130, 236, 0, 0, 174, 0, 0.0, 1, 1, 2]
        f = np.array(list_data, dtype=float)
        accuracy,pred = prdict_heart_disease(f)
        accuracys,preds = prdicts_heart_disease(f)
        accuracyss,predss = prdictss_heart_disease(f)

        # accuracyss,predss = prdictss_heart_disease(list_data)
        patient = Patient.objects.get(user=request.user)
        Search_Data.objects.create(patient=patient, prediction_accuracy=accuracy, prediction_accuracys=accuracys,prediction_accuracyss=accuracyss,result=pred[0],results=preds[0],resultss=predss[0],values_list=list_data)
        ...
        # if preds[0] == 0:
        #         preds = "<span style='color:green'>You are healthy</span>"
        # else:
        #     preds = "<span style='color:red'>You are Unhealthy, Need to Checkup.</span>"
        # if predss[0] == 0:
        #             preds = "<span style='color:green'>You are healthy</span>"
        # else:
        #     predss = "<span style='color:red'>You are Unhealthy, Need to Checkup.</span>"
        return redirect('predict_desease')
    return render(request, 'add_heartdetail.html')

@login_required(login_url="login")
def predict_desease(request):
    s = Search_Data.objects.all().distinct()
